...
cpt(2,1,:)=[1.0, 0.0];
cpt(2,2,:)=[0.0, 1.0];
bnet.CPD{bnet.names('NewICS(And)')}=tabular_CPD(bnet,bnet.names('NewICS(And)'),'CPT',cpt);
clear cpt;
...
cpt(2,2,2,1,:)=[0.0, 1.0];
cpt(2,2,2,2,:)=[0.0, 1.0];
bnet.CPD{bnet.names('ICSCompr(Or)')}=tabular_CPD(bnet,bnet.names('ICSCompr(Or)'),'CPT',cpt);
clear cpt;
